...
######################################################
with open("abbreviationSynonym.json") as fr:
    synonym_dic = json.loads(fr.read())

# with open("finalDictionaryMoreLink.json") as fr:
#     wholeDic_dic = json.loads(fr.read())
wholeDic_dic = MultiDict(dict_path="finalDictionaryMoreLink_",dict_number=2)
with open("definition.json") as fr:
    definition_dic = json.loads(fr.read())  # get the whole dictionary
    ...
    def get(self):
        ...
        if normalization_list is False:  # the term is not in our database
            ...
            if len(normalization_list) == 1:  # only one normalization form
                ...
                template_values = {"name": finding_dic["name"], "finding_dic": json.dumps(finding_dic),
                                   "metaData": metaData}
                writeTemplate(self, template_values, 'template.html')
            else:  # there are more than one normalization forms
                termDefine_dic = getMultipleCandidates(
                    normalization_list)  # get all its potential full names with its wiki link
